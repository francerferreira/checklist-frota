...
const state = {
    ...
    availabilityFilters: { search: "", family: "TODOS" },
    availabilityView: "QUADRO",
...
const elements = {
    ...
    adminCatalogDetail: document.getElementById("admin-catalog-detail"),
    adminCatalogDetailBack: document.getElementById("admin-catalog-detail-back"),
    adminCatalogDetailSection: document.getElementById("admin-catalog-detail-section"),
    ...
    adminCatalogStatus: document.getElementById("admin-catalog-status"),
    adminCatalogSecondaryFilter: document.getElementById("admin-catalog-secondary-filter"),
    adminCatalogRefresh: document.getElementById("admin-catalog-refresh"),
    adminCatalogDetailContent: document.getElementById("admin-catalog-detail-content"),
    ...
    availabilityViewButtons: Array.from(document.querySelectorAll("[data-availability-view]")),
...
const MODULE_REPORT_DEFINITIONS = {
    equipment: {
        ...
        reports: [
            ...
            { label: "OPERAÇÃO", title: "HORÍMETRO", description: "Consultar e registrar leituras por equipamento.", action: "availability" },
...
function adminCatalogSearchable(row, definition) {
    return `${definition.name(row) || ""} ${definition.code(row) || ""} ${row.description || row.legal_name || row.full_name || row.location || ""}`.toLocaleLowerCase("pt-BR");
}
...
function adminCatalogFormFields(catalog, row = {}) {
    ...
    if (catalog === "stock" && state.adminCatalogs.editingKind === "location") {
        ...
        return `<label class="admin-catalog-field-wide"><span>ARMAZÉM *</span><select name="warehouse_id" required>${warehouses.map((warehouse) => `<option value="${Number(warehouse.id)}" ${Number(row.warehouse_id) === Number(warehouse.id) ? "selected" : ""}>${escapeHtml(warehouse.name || warehouse.code)}</option>`).join("")}</select></label>
            <label><span>PRATELEIRA *</span><input name="shelf_code" required value="${value("shelf_code")}" placeholder="P1"></label>
            <label><span>LOCAL *</span><input name="location_code" required value="${value("location_code")}" placeholder="A"></label>
            <label><span>POSIÇÃO *</span><input name="position_code" required value="${value("position_code")}" placeholder="01"></label>
            <label><span>SITUAÇÃO</span><select name="active"><option value="true" ${row.active !== false ? "selected" : ""}>ATIVO</option><option value="false" ${row.active === false ? "selected" : ""}>INATIVO</option></select></label>`;
    }
    if (catalog === "providers") return `
        <label><span>CÓDIGO *</span><input name="code" required value="${value("code")}" placeholder="PROV-001"></label>
        <label><span>NOME *</span><input name="name" required value="${value("name")}" placeholder="Nome principal"></label>
        <label><span>RAZÃO SOCIAL</span><input name="legal_name" value="${value("legal_name")}"></label>
        <label><span>NOME FANTASIA</span><input name="trade_name" value="${value("trade_name")}"></label>
        <label><span>CNPJ / DOCUMENTO</span><input name="tax_id" value="${value("tax_id")}"></label>
        <label><span>CONTATO</span><input name="contact_name" value="${value("contact_name")}"></label>
        <label><span>E-MAIL</span><input type="email" name="email" value="${value("email")}"></label>
        <label><span>TELEFONE</span><input name="phone" value="${value("phone")}"></label>
        ...
        <label><span>PREFERENCIAL</span><select name="preferred"><option value="false" ${row.preferred ? "" : "selected"}>NÃO</option><option value="true" ${row.preferred ? "selected" : ""}>SIM</option></select></label>`;
    if (catalog === "checklist") {
        const checklistType = row.tipo || row.vehicle_type || "cavalo";
        const groupingType = row.tipo_agrupamento || "simples";
        ...
        <label><span>TIPO DE EQUIPAMENTO *</span><select name="tipo"><option value="cavalo" ${selected("cavalo", checklistType)}>CAVALO</option><option value="carreta" ${selected("carreta", checklistType)}>CARRETA</option><option value="carro_simples" ${selected("carro_simples", checklistType)}>CARRO SIMPLES</option><option value="cavalo_auxiliar" ${selected("cavalo_auxiliar", checklistType)}>CAVALO AUXILIAR</option><option value="ambulancia" ${selected("ambulancia", checklistType)}>AMBULÂNCIA</option><option value="caminhao_pipa" ${selected("caminhao_pipa", checklistType)}>CAMINHÃO PIPA</option><option value="caminhao_brigada" ${selected("caminhao_brigada", checklistType)}>CAMINHÃO BRIGADA</option><option value="onibus" ${selected("onibus", checklistType)}>ÔNIBUS</option><option value="van" ${selected("van", checklistType)}>VAN</option></select></label>
        <label><span>ORDEM</span><input type="number" min="1" name="position" value="${value("position") || "1"}"></label>
        <label><span>AGRUPAMENTO</span><select name="tipo_agrupamento"><option value="simples" ${selected("simples", groupingType)}>SIMPLES</option><option value="lado" ${selected("lado", groupingType)}>LADO</option><option value="compartimento" ${selected("compartimento", groupingType)}>COMPARTIMENTO</option></select></label>
    ...
    if (catalog === "materials") return `
        ...
        <label><span>APLICAÇÃO</span><select name="aplicacao_tipo"><option value="ambos">AMBOS</option><option value="cavalo">CAVALO</option><option value="carreta">CARRETA</option></select></label>
        <label><span>CLASSE ABC</span><select name="classe_abc"><option value="A">A</option><option value="B">B</option><option value="C">C</option></select></label>
        <label><span>ESTOQUE MÍNIMO</span><input type="number" min="0" name="estoque_minimo" value="${value("estoque_minimo") || "0"}"></label>
        <label><span>PONTO DE REPOSIÇÃO</span><input type="number" min="0" name="ponto_reposicao" value="${value("ponto_reposicao") || "0"}"></label>
        <label><span>CÓDIGO DO PRODUTO</span><input name="codigo_produto" value="${value("codigo_produto")}"></label>
        <label><span>MARCA</span><input name="marca" value="${value("marca")}"></label>
        <label><span>REFERÊNCIA DO FABRICANTE</span><input name="referencia_manual" value="${value("referencia_manual")}"></label>
        <label><span>NÚMERO DO FABRICANTE</span><input name="numero_fabricante" value="${value("numero_fabricante")}"></label>
        <label><span>FAMÍLIA</span><input name="familia_codigo" value="${value("familia_codigo")}"></label>
        <label><span>SITUAÇÃO</span><select name="ativo"><option value="true" ${row.ativo !== false ? "selected" : ""}>ATIVO</option><option value="false" ${row.ativo === false ? "selected" : ""}>INATIVO</option></select></label>`;
    return `
        <label><span>CÓDIGO *</span><input name="code" required value="${value("code")}" placeholder="MMP-001"></label>
        <label><span>NOME *</span><input name="name" required value="${value("name")}" placeholder="Estoque MMP"></label>
        <label><span>TIPO</span><select name="warehouse_type"><option value="PRINCIPAL">ARMAZÉM PRINCIPAL</option><option value="MMP">ESTOQUE MMP</option></select></label>
        <label><span>LOCALIZAÇÃO</span><input name="location" value="${value("location")}"></label>
        <label><span>SITUAÇÃO</span><select name="active"><option value="true" ${row.active !== false ? "selected" : ""}>ATIVO</option><option value="false" ${row.active === false ? "selected" : ""}>INATIVO</option></select></label>`;
}

function filteredAdminCatalogRows() {
    const definition = adminCatalogDefinition();
    const query = String(elements.adminCatalogSearch?.value || "").trim().toLocaleLowerCase("pt-BR");
    const status = String(elements.adminCatalogStatus?.value || "TODOS");
    const secondary = String(elements.adminCatalogSecondaryFilter?.value || "TODOS");
    return state.adminCatalogs.rows.filter((row) => {
        if (query && !adminCatalogSearchable(row, definition).includes(query)) return false;
        if (status !== "TODOS" && (status === "ATIVO") !== definition.active(row)) return false;
        if (state.adminCatalogs.activeCatalog === "providers") {
            if (secondary === "HOMOLOGADOS" && !row.homologated) return false;
            if (secondary === "PENDENTES" && row.homologated) return false;
            if (secondary === "PREFERENCIAIS" && !row.preferred) return false;
        }
        if (state.adminCatalogs.activeCatalog === "checklist") {
            if (secondary === "COM_FOTO" && !row.foto_path) return false;
...
function renderAdminCatalogDetailRecord(row = state.adminCatalogs.detailRow) {
    ...
    if (state.adminCatalogs.activeCatalog === "providers") {
        elements.adminCatalogDetailContent.innerHTML = `<div class="admin-record-detail"><header><div><span>PROVEDOR ${escapeHtml(row.code || "")}</span><h4>${escapeHtml(row.name || "SEM NOME")}</h4><p>${escapeHtml(row.trade_name || row.legal_name || "Razão social não informada")}</p></div><button class="primary-button" type="button" data-admin-catalog-edit="${Number(row.id)}">EDITAR PROVEDOR</button></header><div class="admin-record-detail-grid"><div><span>CNPJ / DOCUMENTO</span><strong>${escapeHtml(row.tax_id || "NÃO INFORMADO")}</strong></div><div><span>CONTATO</span><strong>${escapeHtml(row.contact_name || "NÃO INFORMADO")}</strong></div><div><span>E-MAIL</span><strong>${escapeHtml(row.email || "NÃO INFORMADO")}</strong></div><div><span>TELEFONE</span><strong>${escapeHtml(row.phone || "NÃO INFORMADO")}</strong></div><div><span>HOMOLOGAÇÃO</span><strong>${row.homologated ? "HOMOLOGADO" : "PENDENTE"}</strong></div><div><span>PREFERENCIAL</span><strong>${row.preferred ? "SIM" : "NÃO"}</strong></div><div class="admin-record-detail-wide"><span>OBSERVAÇÕES</span><strong>${escapeHtml(row.notes || "Nenhuma observação registrada.")}</strong></div></div></div>`;
        return;
    }
    if (state.adminCatalogs.activeCatalog === "checklist") {
...
function renderAdminCatalogForm(row = null) {
    ...
    if (row && form) {
        const selectedValues = {
            ativo: row.ativo !== undefined ? String(Boolean(row.ativo)) : undefined,
            status: row.status,
            tipo: row.tipo || row.vehicle_type,
            tipo_agrupamento: row.tipo_agrupamento,
            warehouse_type: row.warehouse_type,
            active: row.active !== undefined ? String(Boolean(row.active)) : undefined,
            homologated: row.homologated !== undefined ? String(Boolean(row.homologated)) : undefined,
            preferred: row.preferred !== undefined ? String(Boolean(row.preferred)) : undefined,
            login: row.login,
            aplicacao_tipo: row.aplicacao_tipo,
            classe_abc: row.classe_abc,
        };
        Object.entries(selectedValues).forEach(([name, selected]) => {
    ...
    if (state.adminCatalogs.activeCatalog === "checklist" && form) {
        const grouping = form.elements.namedItem("tipo_agrupamento");
        const syncGrouping = () => {
            const grouped = grouping?.value !== "simples";
            ["item_principal", "parte"].forEach((name) => {
                const field = form.elements.namedItem(name);
                const label = field?.closest("label");
                if (label) label.classList.toggle("hidden", !grouped);
                if (field) field.required = grouped && name === "parte";
            });
        };
...
async function loadAdminCatalogAudit(mode = "audit") {
    try {
        const definition = adminCatalogDefinition();
        const entities = state.adminCatalogs.activeCatalog === "stock" ? ["WAREHOUSE", "WAREHOUSE_LOCATION"] : [definition.entity];
        const responses = await Promise.all(entities.map((entity) => apiFetch(`/admin/audit-logs?entidade=${encodeURIComponent(entity)}`)));
        const rows = responses.flatMap((logs) => Array.isArray(logs) ? logs : []).sort((left, right) => String(right.created_at || "").localeCompare(String(left.created_at || "")));
        elements.adminCatalogDetailContent.innerHTML = rows.length ? `<div class="admin-catalog-log-list">${rows.map((log) => `<article class="admin-catalog-log"><strong>${escapeHtml(log.action || log.event_type || "ALTERAÇÃO")}</strong><span>${escapeHtml(log.entity_type || adminCatalogDefinition().entity)} #${escapeHtml(String(log.entity_id || "-"))}</span><small>${escapeHtml(formatAdminDate(log.created_at))} · ${escapeHtml(log.user || "Usuário do sistema")}</small></article>`).join("")}</div>` : `<div class="empty-state"><strong>${mode === "history" ? "NENHUM HISTÓRICO REGISTRADO" : "NENHUM EVENTO DE AUDITORIA"}</strong><span>As próximas alterações deste cadastro aparecerão aqui.</span></div>`;
    } catch (error) {
        renderStateCard(elements.adminCatalogDetailContent, { title: "AUDITORIA INDISPONÍVEL", message: error.message || "Não foi possível consultar os registros.", tone: "error" });
    }
}

async function loadAdminCatalogHistory() {
    const definition = adminCatalogDefinition();
    const row = state.adminCatalogs.detailRow;
    if ((state.adminCatalogs.activeCatalog === "providers" || state.adminCatalogs.activeCatalog === "checklist") && row && definition.historyEndpoint) {
        try {
            const history = await apiFetch(definition.historyEndpoint(Number(row.id)));
            if (state.adminCatalogs.activeCatalog === "providers") {
                const summary = history.summary || {};
                elements.adminCatalogDetailContent.innerHTML = `<div class="admin-catalog-summary"><div><span>SCS</span><strong>${Number(summary.purchase_requests || 0)}</strong></div><div><span>PCS</span><strong>${Number(summary.purchase_orders || 0)}</strong></div><div><span>NFS</span><strong>${Number(summary.invoices || 0)}</strong></div></div><div class="admin-catalog-history-list">${(history.events || []).map((event) => `<article><strong>${escapeHtml(event.type || "EVENTO")}</strong><span>${escapeHtml(event.reference || "-")}</span><small>${escapeHtml(formatAdminDate(event.date))} · ${escapeHtml(event.status || "")}</small></article>`).join("") || `<div class="empty-state"><strong>NENHUM MOVIMENTO DE COMPRAS</strong><span>Este provedor ainda não possui processos relacionados.</span></div>`}</div>`;
            } else {
                const summary = history.summary || {};
                elements.adminCatalogDetailContent.innerHTML = `<div class="admin-catalog-summary"><div><span>CHECKLISTS</span><strong>${Number(summary.checklists || 0)}</strong></div><div><span>OK</span><strong>${Number(summary.ok || 0)}</strong></div><div><span>NÃO CONFORME</span><strong>${Number(summary.nc || 0)}</strong></div></div><div class="admin-catalog-history-list">${(history.events || []).map((event) => `<article><strong>${escapeHtml(event.status || "REGISTRO")}</strong><span>${escapeHtml(event.vehicle || "Equipamento")}</span><small>${escapeHtml(formatAdminDate(event.date))} · ${escapeHtml(event.user || "Usuário")}</small></article>`).join("") || `<div class="empty-state"><strong>NENHUM USO REGISTRADO</strong><span>Este item ainda não foi usado em um checklist.</span></div>`}</div>`;
            }
            return;
        } catch (error) {
...
function setAdminCatalogTab(tab = "list") {
    const selected = ["list", "detail", "form", "history", "audit"].includes(tab) ? tab : "list";
...
async function loadAdminCatalogRecords() {
    ...
    renderStateCard(elements.adminCatalogDetailContent, { title: "CARREGANDO CADASTRO", message: "Consultando registros administrativos.", tone: "loading" });
    try {
        const params = new URLSearchParams(definition.endpoint.split("?")[1] || "");
        const query = String(elements.adminCatalogSearch?.value || "").trim();
        const status = String(elements.adminCatalogStatus?.value || "TODOS");
        if (query) {
            if (state.adminCatalogs.activeCatalog === "employees") params.set("busca", query);
            else params.set("q", query);
        }
        if (state.adminCatalogs.activeCatalog === "checklist") params.set("ativos", status === "TODOS" ? "all" : status === "ATIVO" ? "true" : "false");
        else if (state.adminCatalogs.activeCatalog === "employees") { if (status !== "TODOS") params.set("situacao", status); }
        else if (status !== "TODOS") params.set("status", status);
        ...
        if (state.adminCatalogs.activeCatalog === "stock") {
            ...
            state.adminCatalogs.rows = Array.isArray(rows) ? rows : [];
        }
        setAdminCatalogTab("list");
    } catch (error) {
        renderStateCard(elements.adminCatalogDetailContent, { title: "CADASTRO INDISPONÍVEL", message: error.message || "Tente novamente.", tone: "error" });
    } finally { state.adminCatalogs.loading = false; }
}

function openAdminCatalogDetail(action) {
    if (!hasAdminAccess()) { showToast("SOMENTE ADMIN PODE OPERAR OS CADASTROS.", true); return; }
    ...
    elements.adminCatalogDetailSection.textContent = definition.section;
    elements.adminCatalogDetailTitle.textContent = definition.title;
    elements.adminCatalogDetailDescription.textContent = definition.description;
    elements.adminCatalogNewLocationButton?.classList.toggle("hidden", action !== "stock");
    elements.adminCatalogSearch.value = "";
    elements.adminCatalogStatus.value = "TODOS";
    const secondaryWrap = document.getElementById("admin-catalog-secondary-filter-wrap");
    const secondaryOptions = action === "providers"
...
function closeAdminCatalogDetail() {
    elements.adminCatalogDetail?.classList.add("hidden");
...
async function submitAdminCatalogForm(event) {
    ...
    ["active", "ativo", "homologated", "preferred"].forEach((key) => { if (key in payload) payload[key] = payload[key] === "true"; });
    try {
    if (catalog === "checklist") {
...
async function openAvailabilityMenu(options = {}) {
    ...
    renderStateCard(elements.availabilityList, {
        ...
        message: "Buscando os equipamentos e as últimas leituras de horímetro.",
        tone: "loading",
    });
    try {
        ...
        if (cachedOverview) {
            ...
            renderAvailability();
            showToast("HORÍMETRO OFFLINE CARREGADO. O REGISTRO SERÁ SINCRONIZADO.");
            return;
        }
        elements.availabilityCounter.textContent = "FALHA";
        renderStateCard(elements.availabilityList, {
            title: "NÃO FOI POSSÍVEL CARREGAR O HORÍMETRO",
            message: error.message || "Verifique a conexão e tente novamente.",
            tone: "error",
        });
        showToast(error.message, true);
...
function renderAvailability() {
    ...
    const measured = visibleRows.filter((row) => row.vehicle?.operational_state?.latest_hourmeter != null).length;
    const withoutReading = Math.max(visibleRows.length - measured, 0);
    elements.availabilityCounter.textContent = visibleRows.length === rows.length
        ? `${rows.length} EQUIPAMENTO${rows.length === 1 ? "" : "S"} COM HORÍMETRO`
        : `${visibleRows.length} DE ${rows.length} EQUIPAMENTOS COM HORÍMETRO`;
    ...
    elements.availabilityViewButtons.forEach((button) => {
        const active = String(button.dataset.availabilityView || "QUADRO").toUpperCase() === state.availabilityView;
        button.classList.toggle("is-active", active);
        button.setAttribute("aria-pressed", String(active));
    });
    const viewClass = { QUADRO: "view-quadro", LISTA: "view-lista", "CARTÃO": "view-cartao" }[state.availabilityView] || "view-quadro";
    elements.availabilityList.classList.remove("view-quadro", "view-lista", "view-cartao");
    elements.availabilityList.classList.add(viewClass);
    elements.availabilitySummary.innerHTML = `
        <article><span>EQUIPAMENTOS</span><strong>${visibleRows.length}</strong></article>
        <article><span>COM LEITURA</span><strong>${measured}</strong></article>
        <article><span>SEM LEITURA</span><strong>${withoutReading}</strong></article>
    `;
...
function makeAvailabilityCard(row) {
    ...
    const hasHourmeter = operationalState.latest_hourmeter != null;
    const card = document.createElement("article");
    card.dataset.vehicleId = String(vehicle.id || "");
    card.className = `availability-card hourmeter-card ${hasHourmeter ? "hourmeter-measured" : "hourmeter-missing"} status-${status.toLowerCase()}`;
    card.innerHTML = `
        <header>
            <div>
            ...
            <b>${hasHourmeter ? "COM LEITURA" : "SEM LEITURA"}</b>
        </header>
        ...
        <details class="availability-action-panel availability-action-hourmeter">
            <summary><span>01</span><div><strong>REGISTRAR HORÍMETRO</strong><small>Digite a leitura mostrada no painel.</small></div></summary>
...
elements.availabilityViewButtons.forEach((button) => {
    on(button, "click", () => {
        state.availabilityView = String(button.dataset.availabilityView || "QUADRO").toUpperCase();
        renderAvailability();
    });
});
on(elements.availabilityClearFilters, "click", () => {
    state.availabilityFilters = { search: "", family: "TODOS" };
...
on(elements.adminCatalogRefresh, "click", () => void loadAdminCatalogRecords());
on(elements.adminCatalogSearch, "input", () => {
    if (state.adminCatalogs.activeTab !== "list") return;
    window.clearTimeout(state.adminCatalogs.searchTimer);
    state.adminCatalogs.searchTimer = window.setTimeout(() => void loadAdminCatalogRecords(), 250);
});
on(elements.adminCatalogStatus, "change", () => {
    if (state.adminCatalogs.activeTab === "list") void loadAdminCatalogRecords();
});
on(elements.adminCatalogSecondaryFilter, "change", () => {
...
on(elements.adminCatalogDetailContent, "click", (event) => {
    ...
    if (viewButton) {
        const row = state.adminCatalogs.rows.find((item) => Number(item.id) === Number(viewButton.dataset.adminCatalogView));
        if (row) {
