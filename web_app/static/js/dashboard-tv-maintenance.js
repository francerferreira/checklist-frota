(function () {
    ...
    function renderDashboard(data) {
        ...
        renderTable("materials", ["MATERIAL / EQUIPAMENTO", "STATUS", "PREVISAO / PROVEDOR"], data.materials || [], (item) => `<div class="table-row"><span><strong>${escapeHtml(item.reference || "SEM REFERENCIA")}</strong><small>${escapeHtml(item.description || "Material nao identificado")} · ${escapeHtml(item.vehicle || "SEM EQUIPAMENTO")}</small></span><span>${statusChip(item.status)}</span><span>${escapeHtml(item.expected_date ? formatDate(item.expected_date) : "SEM DATA")}<small>${escapeHtml(item.supplier || "Provedor nao informado")}</small></span></div>`, "Nenhum material bloqueante.");
