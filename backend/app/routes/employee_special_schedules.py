...
def _guard_hr_management():
    if not user_has_management_access(g.current_user):
        return api_response(False, error="Somente admin ou gestor podem lançar escalas especiais.", status_code=403)
    return None
...
def create_special_schedule():
    ...
    try:
        ...
        for entry in entries:
            ...
            dsr_date = _date((entry or {}).get("dsr_date"), "a data da DSR")
            if dsr_date == schedule_date:
                raise ValueError("A DSR precisa ser em um dia diferente da escala especial.")
            if _is_on_vacation(employee.id, schedule_date) or _is_on_vacation(employee.id, dsr_date):
                raise ValueError("Não é permitido escalar ou lançar DSR para colaborador em férias.")
            ...
            if attendance and attendance.occurrence_type != "DSR":
                raise ValueError("Já existe outra ocorrência ativa na data de DSR de um colaborador selecionado.")
            if not attendance:
                attendance = EmployeeAttendanceRecord(
                    ...
                    occurrence_type="DSR",
                    record_status="ATIVO",
                    notes=f"DSR vinculada à escala {schedule_type.lower()} de {schedule_date.isoformat()}.",
                    created_by_user_id=g.current_user.id,
                )
                db.session.add(attendance)
                db.session.flush()
            prepared.append((employee, dsr_date, attendance))

        created = []
        for employee, dsr_date, attendance in prepared:
            schedule = EmployeeSpecialSchedule(
                ...
                dsr_attendance_record_id=attendance.id,
        ...
        db.session.rollback()
        return api_response(False, error="A escala ou a DSR já possui um lançamento para um dos colaboradores.", status_code=409)
